...
begin
    ...
    try
        try
            ...
            Model_TarefasService.Tarefa.NOMETAREFA  := body.GetValue<string>('nometarefa', '');
            Model_TarefasService.Tarefa.RESPONSAVEL  := body.GetValue<string>('responsavel', '');
            Model_TarefasService.Tarefa.STATUS       := body.GetValue<string>('status', '');
            Model_TarefasService.Tarefa.PRIORIDADE   := body.GetValue<integer>('prioridade', 1);
...
begin
    ...
    try
        ...
        Model_TarefasService.Tarefa.ID:=Req.Params['id'].ToInteger;
...
begin
    ...
    try
       ...
       Model_TarefasService.Tarefa.ID := body.GetValue<integer>('id', 0);
       Model_TarefasService.Tarefa.STATUS       := body.GetValue<string>('status', '');

       if   Model_TarefasService.Tarefa.ID>0 then
       begin
